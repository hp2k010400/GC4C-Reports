...
import { shopifyFetchPage, shopifyGetOne, shopifyGraphQL } from '../../lib/shopify.js'
import { sendEmail, REPORT_RECIPIENTS } from '../../lib/mailer.js'
...
function getDateRanges() {
  ...
  // WTD: Monday of current week to today
  ...
  // WTD LY: same Monday last year (364 days = exactly 52 weeks, keeps same day of week)
  const mondayLY = new Date(monday.getTime() - 364 * 86400000)
  const todayLY = new Date(now.getTime() - 364 * 86400000)

  // MTD LY: same month last year, 1st to same day of month
...
async function fetchOrders(locationId, startDate, endDate, full = true) {
  ...
  let pageInfo = null
  const fields = full
    ? 'id,total_price,total_discounts,line_items,refunds'
    : 'id,total_price'
  do {
...
function calcMetrics(orders, variantCosts) {
  let totalSales = 0, totalDiscounts = 0, totalCost = 0, totalRefunds = 0
  ...
  const gross = totalSales + totalDiscounts
  return {
    ...
    orderCount,
    margin: totalSales > 0 ? parseFloat(((totalSales - totalCost) / totalSales * 100).toFixed(1)) : 0,
...
async function fetchReportData(ranges) {
  ...
  const perLocation = await Promise.all(locations.map(async (loc) => {
    const [wtd, mtd, wtdLY, mtdLY] = await Promise.all([
    ...
    return { loc, wtd, mtd, wtdLY, mtdLY }
  }))

  // Batch fetch all variant costs
  const allVariantIds = []
  ...
  return perLocation.map(({ loc, wtd, mtd, wtdLY, mtdLY }) => ({
    store: loc.name,
    wtd: calcMetrics(wtd, variantCosts),
    mtd: calcMetrics(mtd, variantCosts),
...
function buildPosEmail(storeData, ranges) {
  ...
  function sectionTable(period, lyKey, accentColor) {
    const rows = storeData.map(s => {
      ...
      return `<tr>
        ...
        <td style="${num}font-weight:700;color:${d.margin >= 30 ? '#005F2C' : d.margin > 0 ? '#333' : '#aaa'};">${d.margin > 0 ? d.margin + '%' : '—'}</td>
        <td style="${num}">${d.atv > 0 ? fmtGbp(d.atv) : '—'}</td>
        <td style="${num}color:${d.discountPct > 10 ? '#dc2626' : '#333'};">${d.discountPct > 0 ? d.discountPct + '%' : '—'}</td>
        <td style="${num}color:${d.returnPct > 5 ? '#dc2626' : '#333'};">${d.returnPct > 0 ? d.returnPct + '%' : '—'}</td>
      </tr>`
    }).join('')
    ...
    return `<table width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
      ...
      <tbody>
        ...
        <tr style="background:#f7f8fa;border-top:2px solid #e4e4e4;">
          ...
          <td style="${num}font-weight:700;color:${tMargin >= 30 ? '#005F2C' : '#333'};">${tMargin > 0 ? tMargin + '%' : '—'}</td>
          <td style="${num}font-weight:700;">${tAtv > 0 ? fmtGbp(tAtv) : '—'}</td>
          <td style="${num}">${tDisc > 0 ? tDisc + '%' : '—'}</td>
          <td style="${num}">${tRet > 0 ? tRet + '%' : '—'}</td>
        </tr>
      </tbody>
    </table>`
  }
...
<div style="max-width:720px;margin:0 auto;">
  ...
  <div style="background:white;padding:0 0 8px;">
    ...
    ${sectionTable('wtd', 'wtdLYSales', '#005F2C')}
  </div>
  <div style="background:white;padding:0 0 8px;border-top:4px solid #f4f5f7;">
    ...
    ${sectionTable('mtd', 'mtdLYSales', '#1d4ed8')}
  </div>
