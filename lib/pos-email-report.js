...
export function buildPosEmail(storeData, ranges, summaryAtv = {}, summaryReturnPct = {}, staffWtd = [], staffMtd = []) {
  ...
  function staffTable(staffRows, labelText, dateRange) {
    if (!staffRows || staffRows.length === 0) return ''
    // Group by store in STORE_ORDER
    const byStore = {}
    ...
    const storeBlocks = STORE_ORDER
      ...
      .map(storeName => {
        ...
        const storeTotal = members.reduce((a, m) => ({ sales: a.sales + m.totalSales, orders: a.orders + m.orders, gp: a.gp + m.grossProfit }), { sales: 0, orders: 0, gp: 0 })
    ...
    return `<div style="background:white;padding:0 0 8px;border-top:4px solid #f4f5f7;">
      <div class="em-label" style="padding:16px 28px 4px;font-size:12px;font-weight:700;color:#6b21a8;text-transform:uppercase;letter-spacing:0.06em;">
        Staff Breakdown &mdash; ${labelText}
      </div>
...
<div style="max-width:720px;margin:0 auto;">
  ...
  ${staffTable(staffWtd, `Week to Date &mdash; ${fmtShortDate(ranges.wtd.start)} to Today`, `Individual staff sales for the current week`)}
  ${staffTable(staffMtd, `Month to Date &mdash; ${fmtShortDate(ranges.mtd.start)} to Today`, `Individual staff sales for the current month`)}
...
export async function run({ testRecipient } = {}) {
  ...
  const { stores: storeData, summaryAtv, summaryReturnPct, staffWtd, staffMtd } = await fetchReportData(ranges)
  ...
  await sendEmail({
    ...
    html: buildPosEmail(storeData, ranges, summaryAtv, summaryReturnPct, staffWtd, staffMtd),
  })
...
export async function runCustomRange(from, to, { testRecipient } = {}) {
  ...
  const { stores: storeData, summaryAtv, summaryReturnPct, staffWtd, staffMtd } = await fetchReportData(fakeRanges)
  ...
  const html = buildPosEmail(storeData, {
    ...
    today: toDate,
    _customLabel: label,
  }, summaryAtv, summaryReturnPct, staffWtd, staffMtd)
