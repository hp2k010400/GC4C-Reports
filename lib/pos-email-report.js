...
async function fetchReportData(ranges) {
  ...
  return {
    stores: perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
      store: loc.name,
      wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null, sqlWtd.get(loc.name)?.netSales ?? null, sqlWtd.get(loc.name)?.grossMarginPct ?? null, sqlWtd.get(loc.name)?.ordersCount ?? null, sqlWtd.get(loc.name)?.avgOrderValue ?? null, sqlWtd.get(loc.name)?.returnPct ?? null),
      mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null, sqlMtd.get(loc.name)?.netSales ?? null, sqlMtd.get(loc.name)?.grossMarginPct ?? null, sqlMtd.get(loc.name)?.ordersCount ?? null, sqlMtd.get(loc.name)?.avgOrderValue ?? null, sqlMtd.get(loc.name)?.returnPct ?? null),
