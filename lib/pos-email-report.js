...
async function fetchRefundsForRanges(locationId, wtdStart, mtdStart, endDate) {
    ...
    do {
      ...
      for (const order of items) {
        for (const refund of (order.refunds || [])) {
          ...
          if (refundDate > endDate) continue
          for (const tx of (refund.transactions || [])) {
            if (tx.kind !== 'refund' || tx.status !== 'success') continue
            const amount = parseFloat(tx.amount || 0)
            if (refundDate >= wtdStart) wtd += amount
            if (refundDate >= mtdStart) mtd += amount
          }
        }
      }
      pageInfo = nextPageInfo
...
async function fetchReportData(ranges) {
  ...
  const perLocation = await Promise.all(locations.map(async (loc) => {
    const [wtd, mtd, wtdLY, mtdLY, refunds] = await Promise.all([
      ...
      fetchRefundsForRanges(loc.id, ranges.wtd.start, ranges.mtd.start, ranges.todayStr),
    ])
