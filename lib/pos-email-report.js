...
async function fetchOrders(locationId, startDate, endDate, full = true) {
  const orders = []
  let pageInfo = null
  const fields = full ? 'id,total_price,subtotal_price,total_discounts,total_tax,line_items' : 'id,total_price'
  do {
...
function calcMetrics(orders, variantCosts, totalRefunds = 0) {
  let totalSales = 0, totalSubtotal = 0, totalDiscounts = 0, totalCost = 0, totalTax = 0
...
export function buildPosEmail(storeData, ranges) {
  ...
  function sectionTable(period, lyKey) {
    ...
    const t = storeData.reduce((acc, s) => {
      ...
      return acc
    }, { sales: 0, subtotal: 0, tax: 0, cost: 0, discounts: 0, refunds: 0, orders: 0, ly: 0 })
