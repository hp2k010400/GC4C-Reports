...
async function fetchReturnsAndRefundsForRanges(locationId, wtdStart, mtdStart, endDate) {
  // Fetch orders updated since the earlier of wtdStart/mtdStart — Shopify bumps
  // updated_at when a return is processed so this catches all in-period returns
  ...
  while (hasMore) {
    ...
    for (const { node: order } of (ordersData.edges || [])) {
      ...
      if (returnEdges.length > 0) {
        // New Returns workflow: use refunds attached to each Return
        for (const { node: ret } of returnEdges) {
          ...
          let val = 0
          for (const { node: refund } of (ret.refunds?.edges || [])) {
            val += parseFloat(refund.totalRefundedSet?.shopMoney?.amount || 0)
          }
          if (d >= wtdStart) wtd += val
          mtd += val
        }
      } else {
        // Legacy path: order-level refunds (no Return object)
        for (const refund of (order.refunds || [])) {
          const d = (refund.createdAt || '').slice(0, 10)
