...
function calcMetrics(orders, variantCosts, totalRefunds = 0) {
  ...
  return {
    ...
    returnPct: totalSubtotal > 0 ? parseFloat((totalRefunds / totalSubtotal * 100).toFixed(1)) : 0,
  }
}
...
export function buildPosEmail(storeData, ranges) {
  ...
  function sectionTable(period, lyKey) {
    ...
    const t = storeData.reduce((acc, s) => {
      const d = s[period]
      acc.sales += d.totalSales; acc.subtotal += d.totalSubtotal; acc.cost += d.totalCost
      acc.discounts += d.totalDiscounts; acc.refunds += d.totalRefunds
      acc.orders += d.orderCount; acc.ly += s[lyKey]
      return acc
    }, { sales: 0, subtotal: 0, cost: 0, discounts: 0, refunds: 0, orders: 0, ly: 0 })
    const tMargin = t.subtotal > 0 ? parseFloat(((t.subtotal - t.cost) / t.subtotal * 100).toFixed(1)) : 0
    const tAtv = t.orders > 0 ? parseFloat((t.sales / t.orders).toFixed(2)) : 0
    const tGross = t.subtotal + t.discounts
    const tDisc = tGross > 0 ? parseFloat((t.discounts / tGross * 100).toFixed(1)) : 0
    const tRet = t.subtotal > 0 ? parseFloat((t.refunds / t.subtotal * 100).toFixed(1)) : 0
