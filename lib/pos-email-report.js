...
async function fetchShopifyQLSalesData(since, until) {
  const query = `FROM sales SHOW orders_count, gross_sales, discounts, net_sales, shipping_charges, taxes, total_sales, gross_margin WHERE is_pos_sale = true GROUP BY pos_location_name WITH TOTALS SINCE ${since} UNTIL ${until} ORDER BY total_sales DESC LIMIT 1000`
...
function calcMetrics(orders, variantCosts, totalRefunds = 0, overrideTotalSales = null, overrideNetSales = null, overrideMargin = null, overrideOrderCount = null) {
  ...
  return {
    ...
    atv: finalOrderCount > 0 ? parseFloat((finalTotalSales / finalOrderCount).toFixed(2)) : 0,
...
async function fetchReportData(ranges) {
  ...
  return perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
    store: loc.name,
    wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null, sqlWtd.get(loc.name)?.netSales ?? null, sqlWtd.get(loc.name)?.grossMarginPct ?? null, sqlWtd.get(loc.name)?.ordersCount ?? null),
    mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null, sqlMtd.get(loc.name)?.netSales ?? null, sqlMtd.get(loc.name)?.grossMarginPct ?? null, sqlMtd.get(loc.name)?.ordersCount ?? null),
...
export function buildPosEmail(storeData, ranges) {
  ...
  function sectionTable(period, lyKey) {
    ...
    const tAtv = t.sqlOrders > 0 ? parseFloat((t.sales / t.sqlOrders).toFixed(2)) : (t.orders > 0 ? parseFloat((t.sales / t.orders).toFixed(2)) : 0)
...
export async function runCustomRange(from, to, { testRecipient } = {}) {
  ...
  const storeData = await fetchReportData(fakeRanges)
