...
async function fetchShopifyQLSalesData(since, until) {
  ...
  for (const row of rows) {
    ...
    result.set(storeName, { totalSales: parseFloat(row.total_sales || 0) })
  }
  return result
}
...
function calcMetrics(orders, variantCosts, totalRefunds = 0, overrideTotalSales = null) {
...
async function fetchReportData(ranges) {
  ...
  const [sqlWtd, sqlMtd, sqlWtdLY, sqlMtdLY, perLocationRaw] = await Promise.all([
    fetchShopifyQLSalesData(ranges.wtd.start, ranges.wtd.end),
    fetchShopifyQLSalesData(ranges.mtd.start, ranges.mtd.end),
    fetchShopifyQLSalesData(ranges.wtdLY.start, ranges.wtdLY.end),
    fetchShopifyQLSalesData(ranges.mtdLY.start, ranges.mtdLY.end),
  ...
  return perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
    store: loc.name,
    wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null),
    mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null),
