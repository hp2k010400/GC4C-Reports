...
async function fetchRefundsInPeriod(locationId, startDate, endDate) {
  let total = 0
  let pageInfo = null
  do {
    const params = pageInfo ? { page_info: pageInfo } : {
      status: 'any',
      fields: 'id,refunds',
      updated_at_min: new Date(startDate).toISOString(),
      updated_at_max: new Date(endDate + 'T23:59:59').toISOString(),
      location_id: String(locationId),
      limit: 250,
    }
    const { items, nextPageInfo } = await shopifyFetchPage('orders.json', 'orders', params)
    for (const order of items) {
      for (const refund of (order.refunds || [])) {
        const refundDate = (refund.processed_at || '').slice(0, 10)
...
async function fetchReportData(ranges) {
  ...
  const perLocation = await Promise.all(locations.map(async (loc) => {
    const [wtd, mtd, wtdLY, mtdLY, wtdRefunds, mtdRefunds] = await Promise.all([
      ...
      fetchRefundsInPeriod(loc.id, ranges.wtd.start, ranges.wtd.end),
      fetchRefundsInPeriod(loc.id, ranges.mtd.start, ranges.mtd.end),
    ])
    return { loc, wtd, mtd, wtdLY, mtdLY, wtdRefunds, mtdRefunds }
  }))
