...
function calcMetrics(orders, variantCosts, totalRefunds = 0, overrideTotalSales = null, overrideNetSales = null, overrideMargin = null) {
  ...
  return {
    ...
    atv: orderCount > 0 ? parseFloat((finalTotalSales / orderCount).toFixed(2)) : 0,
...
async function fetchReportData(ranges) {
  ...
  return perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
    store: loc.name,
    wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null, sqlWtd.get(loc.name)?.netSales ?? null, sqlWtd.get(loc.name)?.grossMarginPct ?? null),
    mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null, sqlMtd.get(loc.name)?.netSales ?? null, sqlMtd.get(loc.name)?.grossMarginPct ?? null),
...
export function buildPosEmail(storeData, ranges) {
  ...
  function sectionTable(period, lyKey) {
    ...
    const t = storeData.reduce((acc, s) => {
      ...
      acc.sqlNetSales += d.sqlNetSales || 0; acc.sqlGrossProfit += d.sqlGrossProfit || 0
      if (s[lyKey] > 0) acc.lflSales += d.totalSales
      return acc
    }, { sales: 0, subtotal: 0, tax: 0, cost: 0, discounts: 0, loyaltyDiscounts: 0, refunds: 0, orders: 0, ly: 0, lflSales: 0, sqlNetSales: 0, sqlGrossProfit: 0 })
