...
async function fetchShopifyQLSalesData(since, until) {
  const query = `FROM sales SHOW orders, gross_sales, discounts, net_sales, shipping_charges, taxes, total_sales, gross_margin WHERE is_pos_sale = true GROUP BY pos_location_name WITH TOTALS SINCE ${since} UNTIL ${until} ORDER BY total_sales DESC LIMIT 1000`
