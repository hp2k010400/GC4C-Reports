...
function calcMetrics(orders, variantCosts, totalRefunds = 0, overrideTotalSales = null, overrideNetSales = null, overrideMargin = null, overrideOrderCount = null, overrideAtv = null) {
...
async function fetchReportData(ranges) {
  ...
  const [sqlWtd, sqlMtd, sqlWtdLY, sqlMtdLY, perLocationRaw] = await Promise.all([
    ...
    Promise.all(locations.map(async (loc) => {
      ...
      return { loc, wtd, mtd, wtdRefunds: refunds.wtd, mtdRefunds: refunds.mtd }
    })),
  ])
  ...
  return {
    stores: perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
      store: loc.name,
      wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null, sqlWtd.get(loc.name)?.netSales ?? null, sqlWtd.get(loc.name)?.grossMarginPct ?? null, sqlWtd.get(loc.name)?.ordersCount ?? null, sqlWtd.get(loc.name)?.avgOrderValue ?? null),
      mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null, sqlMtd.get(loc.name)?.netSales ?? null, sqlMtd.get(loc.name)?.grossMarginPct ?? null, sqlMtd.get(loc.name)?.ordersCount ?? null, sqlMtd.get(loc.name)?.avgOrderValue ?? null),
      wtdLYSales: sqlWtdLY.get(loc.name)?.totalSales || 0,
      mtdLYSales: sqlMtdLY.get(loc.name)?.totalSales || 0,
    })),
    summaryAtv: {
      wtd: sqlWtd.get('*')?.avgOrderValue || 0,
      mtd: sqlMtd.get('*')?.avgOrderValue || 0,
    },
  }
}
...
export function buildPosEmail(storeData, ranges, summaryAtv = {}) {
  ...
  function sectionTable(period, lyKey, sqlSummaryAtv = 0) {
...
<div style="max-width:720px;margin:0 auto;">
  ...
  <div style="background:white;padding:0 0 8px;">
    ...
    ${sectionTable('wtd', 'wtdLYSales', summaryAtv.wtd || 0)}
  </div>
  <div style="background:white;padding:0 0 8px;border-top:4px solid #f4f5f7;">
    ...
    ${sectionTable('mtd', 'mtdLYSales', summaryAtv.mtd || 0)}
  </div>
...
export async function run({ testRecipient } = {}) {
  ...
  const { stores: storeData, summaryAtv } = await fetchReportData(ranges)
  ...
  await sendEmail({
    ...
    html: buildPosEmail(storeData, ranges, summaryAtv),
  })
...
export async function runCustomRange(from, to, { testRecipient } = {}) {
  ...
  const { stores: storeData, summaryAtv } = await fetchReportData(fakeRanges)
