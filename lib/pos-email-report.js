...
async function fetchShopifyQLSalesData(since, until) {
  const query = `FROM sales SHOW gross_sales, discounts, net_sales, shipping_charges, taxes, total_sales WHERE is_pos_sale = true GROUP BY pos_location_name WITH TOTALS SINCE ${since} UNTIL ${until} ORDER BY total_sales DESC LIMIT 1000`
...
function calcMetrics(orders, variantCosts, totalRefunds = 0, overrideTotalSales = null, overrideNetSales = null) {
  ...
  return {
    ...
    orderCount,
    margin: marginBase > 0 ? parseFloat(((marginBase - totalCost) / marginBase * 100).toFixed(1)) : 0,
...
async function fetchReportData(ranges) {
  ...
  return perLocationRaw.map(({ loc, wtd, mtd, wtdRefunds, mtdRefunds }) => ({
    store: loc.name,
    wtd: calcMetrics(wtd, variantCosts, wtdRefunds, sqlWtd.get(loc.name)?.totalSales ?? null, sqlWtd.get(loc.name)?.netSales ?? null),
    mtd: calcMetrics(mtd, variantCosts, mtdRefunds, sqlMtd.get(loc.name)?.totalSales ?? null, sqlMtd.get(loc.name)?.netSales ?? null),
