...
export function buildPosEmail(storeData, ranges, summaryAtv = {}, summaryReturnPct = {}) {
  ...
  function sectionTable(period, lyKey, sqlSummaryAtv = 0, overrideTotalReturnPct = null) {
    ...
    // Prefer ShopifyQL total, else weighted average from per-store ShopifyQL returnPct
    ...
    const tRet = (overrideTotalReturnPct != null && overrideTotalReturnPct > 0)
      ...
      : (t.sales > 0 && weightedReturn > 0 ? parseFloat((weightedReturn / t.sales).toFixed(1)) : (tNet > 0 ? parseFloat((t.refunds / tNet * 100).toFixed(1)) : 0))
