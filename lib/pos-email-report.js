...
async function fetchShopifyQLStaffData(since, until) {
  const query = `FROM sales SHOW orders, total_sales, gross_profit, gross_margin, average_order_value WHERE is_pos_sale = true GROUP BY staff_member_name, pos_location_name SINCE ${since} UNTIL ${until} ORDER BY total_sales DESC LIMIT 1000`
  const data = await shopifyGraphQL(`{
    shopifyqlQuery(query: ${JSON.stringify(query)}) {
  ...
  const storeBlocks = STORE_ORDER
    ...
    .map(storeName => {
      ...
      const memberRows = members.map(m => `<tr>
        <td class="emc" style="${cell}padding-left:20px;">${m.staff}</td>
      ...
      const subHead = `<tr style="background:#f8f9fa;">
      ...
      return `<tr style="background:#f0f4f0;">
        <td class="emc" colspan="6" style="${cell}font-weight:700;color:#005F2C;font-size:12px;letter-spacing:0.04em;text-transform:uppercase;">${storeName} &mdash; ${fmtGbp(storeTotal.sales)} (${storeTotal.orders} orders)</td>
...
<div style="max-width:720px;margin:0 auto;">
  ...
  ${buildStaffTable(staffMtd, `Month to Date &mdash; ${mtdLabel}`, `Individual staff sales for the current month`)}
