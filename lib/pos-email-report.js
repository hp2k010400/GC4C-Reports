...
async function fetchReturnsAndRefundsForRanges(locationId, wtdStart, mtdStart, endDate) {
  ...
  while (hasMore) {
    ...
    for (const { node: order } of (ordersData.edges || [])) {
      if (order.locationId && order.locationId !== locationGid) continue
      ...
      if (returnEdges.length > 0) {
        ...
        for (const { node: ret } of returnEdges) {
          ...
          for (const { node: rli } of (ret.returnLineItems?.edges || [])) {
            ...
            const lineTotal = parseFloat(rli.fulfillmentLineItem?.discountedTotalSet?.shopMoney?.amount || 0)
            const lineQty = Math.max(rli.fulfillmentLineItem?.quantity || 1, 1)
            val += (lineTotal / lineQty) * returnQty
          }
