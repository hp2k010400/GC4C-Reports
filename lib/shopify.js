...
// Shopify releases a new API version quarterly. Update this when Shopify emails about deprecation (typically ~1 year after release date).
// Check latest versions at: https://shopify.dev/docs/api/usage/versioning
const API = '2025-04'
...
async function shopifyGet(path, params = {}) {
  ...
  if (!res.ok) {
    ...
    const msg = text.trimStart().startsWith('<') ? `Shopify API error ${res.status}` : text
    throw new Error(msg)
  }
