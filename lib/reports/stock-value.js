          ...
          variants(first: 100) {
            edges {
              node {
                ...
                inventoryItem {
                  unitCost { amount }
                  // GC4C has 6 total locations (incl. one inactive one hidden
                  // from the location picker) — first: 5 was silently
                  // truncating and dropping whichever location sorted last
                  // for a given item, which was skewing External Storage's
                  // totals low. 10 gives headroom for future locations too.
