...
export async function resolveProductImage(query) {
  ...
  const clean = (query || '')
    .replace(/[:—–].*$/, '')
    .replace(/^The\s+/i, '')
    // Unwrap (not strip) a parenthetical qualifier — "The case for a mallet
    // (Phantom)" was losing "Phantom", the one actually specific, useful
    // model name in the heading, to a wrong Newport blade match instead
    // (a real bug, found live on a real article — the safety check now
    // correctly blocks it and returns nothing rather than a wrong photo,
    // but nothing isn't the goal either; unwrapping the parens gets the
    // real word into the query at all).
    .replace(/[()]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
