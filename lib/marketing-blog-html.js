...
// sections[i].tables: [ [[cell,...], ...], ... ] — a section can contain
// more than one real table (one doc had three under a single H2). Each
// table's first row is treated as its header. Rendered as actual <table>
// elements, not squashed into prose.
...
@media(max-width:600px){.gc4c-post .gc4c-img-frame{max-width:100%;}}
  ...
  ;(sections || []).forEach(s => {
    ...
    for (const table of (s.tables || [])) if (table && table.length) parts.push(renderTable(table))
    parts.push('</div>')
  })
