...
// DPD (and most couriers) pay compensation per KG of declared/scanned weight,
// not a flat amount — confirmed with Phil Barron (Returns) 2026-08-11:
// "£12 per kilo of declared weight or if they have a scan weight... so if
// something costs us £150 and they pay us £120 we lose £30" (i.e. a 10kg item).
export const DPD_RATE_PER_KG = 12

// GC4C always declares parcels at 10kg regardless of actual weight, and DPD
// pays out on that in most cases (confirmed with Phil Barron 2026-08-11) -
// so there isn't really a "real" per-parcel weight to look up, this is it.
export const DEFAULT_WEIGHT_KG = 10

// Expected payout from the courier for a claim, based on weight — null if no
// weight is recorded yet (most historical rows, since the old sheet never
// tracked this).
export function expectedPayout(row) {
  const kg = row?.weight_kg
  if (kg == null || kg === '') return null
  return Math.round(Number(kg) * DPD_RATE_PER_KG * 100) / 100
}

// How much of the item's cost we stand to lose even after the claim pays out
// — null unless both weight and cost are known. Negative means the payout
// actually covers the full cost (no loss).
export function expectedShortfall(row) {
  const payout = expectedPayout(row)
  if (payout == null || row?.cost == null || row.cost === '') return null
  return Math.round((Number(row.cost) - payout) * 100) / 100
}
